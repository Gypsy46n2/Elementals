...
func is_enemy(other: Node) -> bool:
	...
	if not other_faction_comp:
		# Fallback
		if other.get("is_friendly") != null:
			return not _is_friendly_to_faction(other.is_friendly)
		return false
